...
// Buat sekarang Data coman disimpan di memory restart server = data hilang
...
let schedules = [
	...
	{ id: 2, title: 'Seminar Informatika', start: '2025-08-28', end: '2025-08-29' },
	{ id: 3, title: 'Ujian Praktikum', start: '2025-09-05', allDay: true }
];
let articles = [
	{ id: 1, title: 'Tips Memilih Laptop untuk Mahasiswa', slug: 'tips-memilih-laptop', excerpt: 'Memilih laptop yang tepat penting bagi mahasiswa...', body: 'Konten lengkap artikel...', createdAt: new Date() }
];
...
app.post('/api/schedules', requireAdmin, (req, res) => {
	...
	if (!title || !start) return res.status(400).json({ error: 'title & start wajib' });
	const id = Date.now();
...
app.get('/api/articles', (_req, res) => {
	res.json(articles.map(a => ({ id: a.id, title: a.title, slug: a.slug, excerpt: a.excerpt, createdAt: a.createdAt })));
});
...
app.post('/api/articles', requireAdmin, (req, res) => {
	...
	if (!title || !body) return res.status(400).json({ error: 'title & body wajib' });
	const id = Date.now();
	const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
	const art = { id, title, slug, body, excerpt: excerpt || body.slice(0, 100) + '...', createdAt: new Date() };
...
app.post('/api/reservations', (req, res) => {
	...
	if (!name || !email || !purpose || !start || !end) return res.status(400).json({ error: 'Field wajib belum lengkap' });
	const id = Date.now();
	const reservation = { id, name, email, purpose, start, end, status: 'pending', createdAt: new Date() };
	reservations.push(reservation);
	res.status(201).json({ message: 'Reservasi diterima (pending)', reservation });
});
app.patch('/api/reservations/:id', requireAdmin, (req, res) => {
	...
	if (status && ['pending', 'approved', 'rejected'].includes(status)) {
		...
		if (status === 'approved') {
			...
			schedules.push({ id: Date.now(), title: `Reservasi: ${r.purpose}`, start: r.start, end: r.end });
		}
	}
	res.json(r);
});
