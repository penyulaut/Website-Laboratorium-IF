...
export default function AdminPanel() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/status`)
      ...
      .then((d) => {
        setStatus(d);
        setStatusForm({ open: d.open, note: d.note || STATUS_TEMPLATES[0] });
      });
  }, [API_BASE]);
        ...
        {token && (
          <div className="space-y-10">
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2 text-sm">
                  <input
                    type="checkbox"
                    checked={statusForm.open}
                    onChange={(e) =>
                      setStatusForm((f) => ({ ...f, open: e.target.checked }))
                    }
                  />{" "}
                  Buka?
                </label>
              ...
              {status && (
                <p className="text-xs text-zinc-500">
                  Update terakhir: {new Date(status.updatedAt).toLocaleString()}
                </p>
              )}
            </form>
            ...
            <form
              ...
              className="bg-zinc-800 p-6 rounded-xl space-y-3"
            >
              <h2 className="text-lg font-semibold text-violet-400">
                Tambah Artikel
              </h2>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setArticleForm((f) => ({ ...f, body: e.target.value }))
                }
                className="w-full p-2 rounded bg-zinc-900 border border-zinc-700"
              />
              <input
                ...
                onChange={(e) =>
                  setArticleForm((f) => ({ ...f, excerpt: e.target.value }))
                }
                className="w-full p-2 rounded bg-zinc-900 border border-zinc-700"
              />
              <button className="bg-violet-700 hover:bg-violet-600 px-4 py-2 rounded">
            ...
            >
              ...
              <input
                ...
                onChange={(e) =>
                  setScheduleForm((f) => ({ ...f, title: e.target.value }))
                }
                className="w-full p-2 rounded bg-zinc-900 border border-zinc-700"
              />
              <DateTimeSlotPicker
