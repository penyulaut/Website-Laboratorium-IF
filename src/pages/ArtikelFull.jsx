    ...
    <section className="py-9" id="articles">
      <div className="mx-auto px-4">
        ...
        <div className="flex items-center gap-3 mb-4">
          <button
            onClick={load}
            className="text-xs bg-zinc-800 hover:bg-violet-600 px-3 py-1 rounded"
          >
            Refresh
          </button>
          {!loading && (
        ...
        {!loading && articles.length > 0 && (
          <div className="grid md:grid-cols-3 gap-5">
            {articles.map((a) => (
              <div
                key={a._id}
                className="bg-zinc-800 rounded-xl hover:shadow-lg transition cursor-pointer flex flex-col"
              ...
              >
                ...
                <div className="p-5 flex-1 flex flex-col">
                  <h2 className="text-xl font-semibold text-violet-600 mb-2 line-clamp-2">
                    {a.judul}
                  </h2>
                  <p className="text-sm text-zinc-300 line-clamp-3 flex-1">
                    {a.isi?.slice(0, 120)}
                    {a.isi && a.isi.length > 120 ? "…" : ""}
                  </p>
                  <p className="text-[11px] text-zinc-500 mt-3">
                    {a.Tanggal ? new Date(a.Tanggal).toLocaleDateString() : ""}
                  </p>
                </div>
              </div>
            ))}
        ...
        {selected && (
          <div className="mt-8 bg-zinc-900 p-6 rounded-xl">
            ...
            <p className="text-xs text-zinc-500 mb-4">
              {selected.Tanggal
                ? new Date(selected.Tanggal).toLocaleString()
                : ""}
            </p>
            {selected.gambar && (
